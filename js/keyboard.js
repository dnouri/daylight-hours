const KeyboardManager = {
    ...
    setupAccessibility() {
        // Add ARIA labels
        document.getElementById('location-search').setAttribute('aria-label', 'Search for location');
        document.getElementById('use-my-location').setAttribute('aria-label', 'Use current location');
    ...
    updateChipAccessibility() {
        document.querySelectorAll('.location-chip').forEach((chip, index) => {
            ...
            chip.setAttribute('aria-label', `Location ${index + 1}: ${chip.querySelector('.chip-name').textContent}`);
