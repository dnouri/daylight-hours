const App = {
  ...
  showBottomSheet(dataPoint, allLocationData) {
    ...
    const locationsEl = this.bottomSheet.querySelector(
      '.bottom-sheet-locations'
    );
    ...
    const primaryLocation = allLocationData.find(
      (item) => item.location.isPrimary
    );
  ...
  ensureChartVisible() {
    ...
    if (chartBottom > availableSpace) {
      ...
      const targetTop = Math.max(
        0,
        chartsRect.top - (chartBottom - availableSpace) - 20
      );
  ...
  calculateYearData(lat, lng) {
    ...
    if (data.length > 0) {
      ...
      if (
        timesBefore.sunrise &&
        timesBefore.sunset &&
        !isNaN(timesBefore.sunrise) &&
        !isNaN(timesBefore.sunset)
      ) {
        const daylightBefore =
          (timesBefore.sunset - timesBefore.sunrise) / (1000 * 60 * 60);
    ...
    if (today) {
      ...
      if (today.isPolarExtreme) {
        document.getElementById('sunrise-time').textContent =
          today.daylight === 24 ? 'Polar Day' : 'No sunrise';
        document.getElementById('sunset-time').textContent =
          today.daylight === 24 ? 'No sunset' : 'Polar Night';
      } else {
        document.getElementById('sunrise-time').textContent = this.formatTime(
          today.sunrise,
          timezoneOffset
        );
        document.getElementById('sunset-time').textContent = this.formatTime(
          today.sunset,
          timezoneOffset
        );
      }
      ...
      document.getElementById('daylight-hours').textContent =
        this.formatDuration(today.daylight);

      // Update daily change
      const changeText =
        today.change > 0
          ? `+${today.change.toFixed(1)}`
          : today.change.toFixed(1);
      ...
      if (solarNoonEl && solarAltitudeEl) {
        solarNoonEl.textContent = this.formatTime(
          today.solarNoon,
          timezoneOffset
        );
  ...
  renderChart(datasets) {
    ...
    const primaryDataset =
      datasets.find((d) => d.location.isPrimary) || datasets[0];
      ...
      .on('touchstart', function (event) {
        ...
        if (nearestPoint) {
          ...
          if (self.isMobile) {
            ...
            const allLocationData = datasets
              .map((dataset, idx) => {
  ...
  updateTooltip(dataPoint, datasets, xScale, yScale, g, event) {
    ...
    allLocationData.forEach((item) => {
      ...
      const currentIndex = datasets
        .find((ds) => ds.location === item.location)
      ...
      if (monthlyData) {
        const monthlyClass =
          monthlyData.totalChange > 0 ? 'positive' : 'negative';
    ...
    // Show tooltip
    this.tooltip.html(tooltipContent).style('opacity', 0.95);
  ...
  renderAltitudeChart(datasets, xScale, parentMargin, parentWidth) {
    ...
    const margin = {
      top: 25,
      right: parentMargin.right,
      bottom: 30,
      left: parentMargin.left,
    };
    ...
    g.append('g').attr('class', 'y-axis altitude-axis').call(yAxis);
