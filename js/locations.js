const LocationManager = {
  ...
  async addDefaultLocation() {
    ...
    if (window.TimezoneManager) {
      try {
        const tzInfo = await window.TimezoneManager.getTimezone(
          location.lat,
          location.lng
        );
  ...
  addLocation(location, shouldSave = true) {
    ...
    const exists = this.locations.find(
  ...
  renderLocationChips() {
    ...
    // Get colors from App
    const colors = window.App
      ? window.App.locationColors
